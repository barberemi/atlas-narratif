...
export function answerQuery(question, data = {}) {
  ...
  if (/\b(où est|ou est|où se trouve|where is)\b/.test(q) && candidateName) {
    ...
    if (res.type === 'character') {
      ...
      if (last) {
        ...
        return { intent: 'where', answer: `${res.entity.name} est vu·e pour la dernière fois à ${loc?.name ?? 'un lieu inconnu'} (chapitre ${last.chapter}).`, matches: [res.entity, ...(loc ? [loc] : [])] };
      }
      return { intent: 'where', answer: `Aucun lieu connu pour ${res.entity.name} (aucun événement localisé).`, matches: [res.entity] };
    }
    return { intent: 'where', answer: `${res.entity.name} est un·e ${res.type}, pas un personnage localisable par trajet.`, matches: [res.entity] };
  }
  ...
  if (/\bobjets?\b|\bporte\b|\bcarries\b|\bheld by\b/.test(q) && candidateName) {
    ...
    if (held.length === 0) return { intent: 'objects', answer: `${res.entity.name} ne détient aucun objet référencé.`, matches: [res.entity] };
    return { intent: 'objects', answer: `${res.entity.name} détient : ${held.map(o => o.name).join(', ')}.`, matches: held };
  }
  ...
  if (/\b(qui est|who is|c'est qui)\b/.test(q) && candidateName) {
    ...
    return { intent: 'who', answer: `${e.name} — ${bits.join(' ') || 'aucune description.'}`, matches: [e] };
  }
  ...
  if (res) {
    const e = res.entity;
    return { intent: 'lookup', answer: `${e.name} (${res.type}) — ${e.description ?? 'aucune description.'}`, matches: [e] };
  }
