...
- Relations   : préfixe \`rel_\` + slug source_cible en snake_case (ex: \`rel_aragorn_arwen\`)
...
### relationsDB[]
Relations **explicites** entre deux entités (noyau typé OU custom) : filiations, alliances, allégeances, appartenances non couvertes ailleurs, etc. À réserver aux liens nommés/structurants du texte — ne pas dupliquer ce qui est déjà porté par un champ (\`origin\`, \`currentHolder\`, \`affiliation\`…).
\`\`\`
{
  id: string,                      // préfixe "rel_" + slug (ex: "rel_aragorn_arwen")
  sourceId: string,                // id de l'entité source (character/location/object/custom)
  sourceType: "character" | "location" | "object" | "custom",
  targetId: string,                // id de l'entité cible
  targetType: "character" | "location" | "object" | "custom",
  label: string | null,            // libellé libre du lien (ex: "père de", "allié de"), sinon null
  directed: boolean                // true = orienté (source→cible), false = symétrique
}
\`\`\`
...
13. **Relations** : dans \`relationsDB\`, \`sourceId\` et \`targetId\` doivent correspondre à des \`id\` existants (characters/locations/objects/customEntitiesDB) et \`sourceType\`/\`targetType\` à leur nature. Réserve-les aux liens explicites du texte non déjà exprimés par un champ.
