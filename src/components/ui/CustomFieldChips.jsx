...
export default function CustomFieldChips({ fields, max = 6 }) {
  // Les clés préfixées `__` sont internes (ex. `__links` = wikilinks résolus par
  // l'import Obsidian) — jamais affichées à l'auteur.
  const entries = Object.entries(fields ?? {})
    .filter(([k, v]) => !k.startsWith('__') && displayVal(v) !== '');
