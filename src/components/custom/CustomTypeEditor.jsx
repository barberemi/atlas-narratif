      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <Field label={t('customType.fieldSchema')}>
          ...
          <div className="space-y-1.5">
            {data.fieldSchema.length > 0 && (
              <div className="flex items-center gap-1.5 px-0.5 text-[9px] font-bold uppercase tracking-[0.12em] text-atlas-mute">
                <span className="w-[34%]">{t('customType.fieldKey')}</span>
                <span className="flex-1 min-w-0">{t('customType.fieldLabel')}</span>
                <span className="w-[76px]">{t('customType.fieldTypeCol')}</span>
                <span className="w-6" />
              </div>
            )}
