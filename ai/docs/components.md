...
### Deep-link « aller pile sur un élément » (scroll + flash)
Convention `?focus=<id>` sur les pages cibles pour arriver directement sur un élément précis (depuis les sources/liens du chat, etc.) : la page scrolle dessus et le fait clignoter ~2,5 s.
- **`src/hooks/useFocusFlash.js`** : `useFocusFlash(ready)` lit `?focus`, retient l'id ciblé une fois les données montées puis purge le param (`replace`). `useFlashScroll(flash)` renvoie `{ ref, flashing }` : le `ref` `scrollIntoView({ block:'center', inline:'center' })` (le `inline` couvre le scroll horizontal de la timeline), et `flashing` ne passe à vrai qu'une fois le scroll **stabilisé** (détection rAF, filet de sécurité 1,5 s) pour que le scintillement soit vu en entier même quand la cible est loin. `useSettleFlash(active, ref, scroll?)` est la brique sous-jacente (réutilisée telle quelle par les cartes Lore, qui gèrent leur propre scroll).
- Animation `.atlas-flash` (keyframe `atlasFlash`, `src/index.css`) : anneau vert sauge pulsé 3×, respecte `prefers-reduced-motion`.
- Les URLs sont produites par `entityUtils` : `hrefForEntity`/`entityHrefById` ajoutent `&focus=<id>` (entités Lore) ; `chatSourceHref` ajoute `?focus=<id>` pour les types adressables (`event`, `plant`, `thread`, `hero`, `incoherence`) — `beat` (= un chapitre) et `note` atterrissent sur la page sans focus.
- Câblage par page : les cartes reçoivent `flash={item.id === flashId}` (`DarkCard` accepte un prop `flash`). Les pages avec filtres réinitialisent ce qui pourrait masquer la cible quand un focus arrive (Timeline : vue chapitres + filtres/tome ; Plants : vue liste + filtres ; Incohérences : filtres). Hero : le focus porte un id d'**entrée** → sélectionne son personnage puis flashe l'étape (`stageKey`).
...
## src/components/chat/
### `ChatPanel`
Page `/chat`, **multi-discussions** : colonne gauche `ChatThreadList` (threads persistés par projet via `useChatStore`/localStorage), colonne droite la conversation active. Niveau 1 : réponses déterministes locales via `src/chat/router.js` (les noms d'entités cités sont émis en wikilinks `[[Nom]]` → liens cliquables) ; niveau 2 (toggle « recherche approfondie ») : `POST /projects/:id/ask` (provider mock par défaut). Les deux niveaux rendent le texte via `AnswerText` et affichent les **sources** en chips cliquables (→ fiche de l'entité ou `?focus=<id>` sur la vue dédiée). Chaque chip est **codée par type** via `chatSourceMeta()` (`entityUtils`) : couleur + icône (les icônes reprennent celles de la nav — `event`/`plant`/`thread`/`hero`/`warning`… — et `ENTITY_ICONS` pour les entités) ; le tooltip affiche `Type · Nom` (i18n `chat.sourceType.*`). **Portées de recherche** (chips `Tous / Personnages / Lieux / Objets / Événements / Intrigue / Notes / Incohérences / Custom`, cf. `src/chat/scopes.js`) : le niveau 2 indexe **toutes** les sources du projet (entités, timeline, STC, plants, fils, hero, notes, incohérences) et restreint le retrieval à la portée choisie ; en « Tous », un indice « recherche surtout dans : X » affiche le type dominant des résultats. À l'activation du mode approfondi, `warmProject()` pré-chauffe le cache d'embeddings (fire-and-forget) pour éviter le cold-start Ollama.
### `ChatThreadList` (chat/)
