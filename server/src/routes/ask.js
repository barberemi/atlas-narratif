 ...
 *      (retrieval par recouvrement de mots — simple, déterministe).
 ...
 * renvoyé au client hormis la réponse + les ids cités.
 *
 * TODO : embeddings + index vectoriel pour un vrai retrieval sémantique ;
 *   réutiliser la logique de GlobalSearch ; garde-fous de coût (voir provider.js).
 */
...
export async function answerAsk({ projectId, question, topK = 5 }) {
  ...
  const result = { answer, citations, provider: provider.name, degraded: degraded ?? false, context: ranked.map(p => ({ id: p.id, name: p.name, type: p.type })) };
