...
async function _doSeed(tx, projectId, meta, data, onProgress, { userId, deviceId } = {}) {
  const {
    ...
    customTypesDB = [], customEntitiesDB = [], relationsDB = [],
  } = data;
  ...
  for (const hj of heroJourneyDB) {
    // ⚠️ project_id DOIT faire partie de l'id : hero_journey_entries a une PK
    // globale (id seul), donc un id non scopé (hj_<char>_<stage>) entre en
    // collision entre projets et fait no-op le seed via ON CONFLICT dès qu'un
    // premier projet a été seedé. → scoper par projet.
    const id = `hj_${projectId}_${hj.characterId}_${hj.stageKey}`;
    await tx`
  ...
  for (const rel of relationsDB) {
    await tx`
      INSERT INTO entity_relations
        (id, project_id, source_id, source_type, target_id, target_type, label, directed, source)
      VALUES (${rel.id ?? `rel_${projectId}_${done}`}, ${projectId},
              ${rel.sourceId}, ${rel.sourceType}, ${rel.targetId}, ${rel.targetType},
              ${e(rel.label ?? null)}, ${rel.directed ?? true}, ${rel.source ?? 'import'})
      ON CONFLICT DO NOTHING
    `;
    done++;
  }
