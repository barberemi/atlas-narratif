...
- `useRelationStore` — relations explicites entre entités (Niveau 3)
- `useChatStore` — discussions de chat multi-threads (persistées localStorage par projet)

**Tables SQL principales** : `projects`, `volumes`, `characters`, `locations`, `objects`, `timeline_events`, `event_entities`, `incoherences`, `stc_chapters`, `arc_points`, `character_journeys`, `chapter_notes`, `character_arc_axes`, `character_arc_points`, `plant_payoffs`, `narrative_threads`, `groups`, `character_groups`, `hero_journey_entries`, `custom_entity_types`, `custom_entities`, `entity_relations`
